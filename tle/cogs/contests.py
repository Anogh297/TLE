...
class Contests(commands.Cog):
    ...
    def _filter_rated_only_contestant_data(handles, ranklist):
        ...
        rated_contestants = [handle for handle in handles if ranklist.standing_by_id.get_correct_handle(handle) in ranklist.delta_by_handle]
    ...
    def _make_vc_rating_changes_embed(guild, contest_id, change_by_handle):
        ...
        member_change_pairs = [
            (member, change_by_handle[handle]) for member, handle in member_handle_pairs if member is not None and handle in change_by_handle
        ]
    ...
    async def _watch_rated_vc(self, vc_id: int):
        ...
        async def has_running_subs(handle):
            return [
                ...
                if sub.verdict == "TESTING" and sub.problem.contestId == vc.contest_id and sub.relativeTimeSeconds <= vc.finish_time - vc.start_time
            ]
    ...
    async def vcperformance(self, ctx, *members: discord.Member):
        ...
        plt.ylim(min_rating - 100, max_rating + 200)
        labels = [gc.StrWrap("{} ({})".format(member_display_name, ratingbefore)) for member_display_name, rating_data in plot_data.items()]
        plt.legend(labels, loc="upper left", prop=gc.fontprop)
