...
def complete_duel(duelid, guild_id, win_status, winner, loser, finish_time, score, dtype):
    ...
    rc = cf_common.user_db.complete_duel(duelid, guild_id, win_status, finish_time, winner.id, loser.id, delta, dtype)
    if rc == 0:
...
class Dueling(commands.Cog):
    ...
    async def challenge(self, ctx, opponent: discord.Member, *args):
        ...
        seen = {
            name for userid in userids for name, in cf_common.user_db.get_duel_problem_names(userid, ctx.guild.id)
        ...
        duelid = cf_common.user_db.create_duel(challenger_id, challengee_id, issue_time, problem, dtype, ctx.guild.id)
    ...
    async def _get_solve_time(self, handle, contest_id, index):
        subs = [
            ...
            if (sub.verdict == "OK" or sub.verdict == "TESTING") and sub.problem.contestId == contest_id and sub.problem.index == index
        ]
    ...
    async def _check_duel_complete(self, guild, channel, data, isAutoComplete=False):
        ...
        problem = [prob for prob in cf_common.cache2.problem_cache.problems if prob.name == problem_name]
        ...
        if highrated_timestamp and lowrated_timestamp:
            ...
            if current_duration >= lowerrated_duration:  # we can make a decision, higher rated won
                ...
                time_remaining_formatted = cf_common.pretty_time_format(time_remaining, always_seconds=True)
                if not isAutoComplete:
                    await channel.send(
        ...
        def make_line(entry):
            ...
            if winner != Winner.DRAW:
                ...
                if winner == None and loser == None:
                    return (
                        f"{idstr if show_id else str()}[{name}]({problem.url}) [{problem.rating}] won by [unknown] vs [unknown] {when} in {duel_time}"
                    )
                if loser == None:
    ...
    async def ranklist(self, ctx):
        ...
        users = [(ctx.guild.get_member(user_id), rating) for user_id, rating in cf_common.user_db.get_duelists(ctx.guild.id)]
        users = [
    ...
    @commands.check_any(constants.is_me(), commands.has_any_role(constants.TLE_ADMIN, constants.TLE_MODERATOR))
    ...
    async def rating(self, ctx, *members: discord.Member):
        ...
        labels = [
            gc.StrWrap("{} ({})".format(ctx.guild.get_member(duelist).display_name, rating_data[-1][1])) for duelist, rating_data in plot_data.items()
        ]
