...
class Codeforces(commands.Cog):
    ...
    async def upsolve(self, ctx, choice: int = -1):
        ...
        problems = [prob for prob in cf_common.cache2.problem_cache.problems if prob.name not in solved and prob.contestId in contests]
    ...
    async def stalk(self, ctx, *args):
        ...
        def make_page(chunk):
            ...
            title = "{} solved problems by {}".format("Hardest" if hardest else "Recently", ", ".join(handlesWithUrl))
    ...
    async def vc(self, ctx, *args: str):
        ...
        recommendations = {
            ...
            and not any(cf_common.is_contest_writer(contest.id, handle) for handle in handles)
        }
        ...
        recommendations.sort(
            key=lambda contest: cf_common.cache2.contest_cache.get_contest(contest).startTimeSeconds,
            reverse=True,
        )
    ...
    async def fullsolve(self, ctx, *args: str):
        ...
        contests = [
            contest
            for contest in cf_common.cache2.contest_cache.get_contests_in_phase("FINISHED")
